...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"soil_1\":\"%d\",", Data_init.soil_1); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"somg\":\"%d\",", Data_init.somg); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"wormnum\":\"%d\",",Data_init.worm); 
	strcat(buf, text);
...
unsigned char Mqtt_Pwd_Info(char *buf)
{
	...
	//有效数据
	memset(text, 0, sizeof(text));
	sprintf(text, "\"pump\":\"%d\",", device_state_init.relay1); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"water\":\"%d\",",device_state_init.relay2); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"fan\":\"%d\",", device_state_init.relay3); 
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"soil_v\":\"%d\",", threshold_value_init.soil_value); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"temp_v\":\"%d\",", threshold_value_init.temp_value); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"light_v\":\"%d\",", threshold_value_init.light_value); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"somg_v\":\"%d\",", threshold_value_init.somg_value); 
	strcat(buf, text);
